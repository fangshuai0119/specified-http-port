...
public class PoolingHttpClientConnectionManager
    ...
    public PoolingHttpClientConnectionManager(
        ...
        this(socketFactoryRegistry, null, null, null, -1, TimeUnit.MILLISECONDS, null);
    }

    public PoolingHttpClientConnectionManager(
            final Registry<ConnectionSocketFactory> socketFactoryRegistry, final PortStrategy portStrategy) {
        this(socketFactoryRegistry, null, null, null, -1, TimeUnit.MILLISECONDS, portStrategy);
    }
    ...
    public PoolingHttpClientConnectionManager(
        ...
        final HttpConnectionFactory<HttpRoute, ManagedHttpClientConnection> connFactory,
        final long timeToLive, final TimeUnit timeUnit, final PortStrategy portStrategy) {
        super();
    ...
    public void connect(
        ...
        if (portStrategy == null) {
            this.connectionOperator.connect(
                    ...
                    connectTimeout, resolveSocketConfig(host), context);
        } else {
            this.connectionOperator.connect(conn, host,
                    // 使用指定端口号访问
                    new InetSocketAddress(portStrategy.getPort()), connectTimeout, resolveSocketConfig(host), context);
        }
    }
     ...
     * be re-validated prior to being {@link #leaseConnection(Future,
     *   long, TimeUnit) leased} to the consumer. Non-positive value passed
     ...
     *
     * @see #leaseConnection(Future, long, TimeUnit)
     *
     * @since 4.4
     */
